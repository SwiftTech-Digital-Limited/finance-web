...
describe("API client", () => {
  it("unwraps success envelopes", async () => {
    ...
    await expect(
      apiRequest<{ ok: boolean }>("/health", { auth: false }),
  ...
  it("preserves structured error details", async () => {
    ...
    const error = await apiRequest("/x", { auth: false }).catch(
      (caught) => caught,
    );
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.endsWith("/auth/refresh")) {
          refreshCalls += 1;
          return new Response(
            JSON.stringify({ success: true, data: { accessToken: "fresh" } }),
  ...
  it("omits absent filters and serializes exact minor-unit names", () => {
    ...
    ).toBe("?page=1&minAmountMinor=250000");
  });
});
