  ...
  return (
    <Link className="account-card" href={`/accounts/${account._id}`}>
      ...
      <div className="resource-balance">
        ...
        <MoneyAmount
          value={
            account.currentBalanceMinor ?? account.openingBalanceMinor ?? 0
          }
...
export function BucketCard({ bucket }: { bucket: Bucket }) {
  ...
  return (
    <Link className="bucket-card" href={`/buckets/${bucket._id}`}>
      <div className="resource-card-top">
        <span
          className={cn("bucket-symbol", `bucket-${bucket.type || "custom"}`)}
        >
          <CircleDot />
        </span>
      ...
      <div>
        <p className="resource-kicker">{bucket.type || "Purpose"}</p>
        <h3>{bucket.name}</h3>
      </div>
      <div className="resource-balance">
        <span>Available</span>
...
export function TransactionRow({
  ...
  event: FinancialEvent | Transaction;
  linked?: boolean;
}) {
  const financialEvent = event as FinancialEvent;
  const reallocation =
    financialEvent.eventKind === "reallocation" ||
    Boolean(financialEvent.reallocation);
  const transfer =
  ...
  const title = reallocation
    ...
    : transfer
      ...
      : event.description ||
        (income
          ? refName(event.incomeSource || event.incomeSourceId, "Income")
          : expense
            ? refName(event.category || event.categoryId, "Expense")
            : "Adjustment");
  ...
  const date =
    ...
    event.createdAt ||
    financialEvent.reallocation?.date ||
    financialEvent.reallocation?.createdAt;
  const amount =
    event.amountMinor ?? financialEvent.reallocation?.amountMinor ?? 0;
  const Icon = reallocation
    ? FolderInput
    : transfer
      ? ArrowLeftRight
      : income
  ...
  const body = (
    <div
      className={cn("transaction-row", event.status === "void" && "void-row")}
    >
      <span
        className={cn(
          "activity-icon",
  ...
  return linked && event._id ? (
    <Link href={`/transactions/${event._id}`}>{body}</Link>
  ) : (
    body
  );
}
    ...
    <section className="allocation-breakdown">
      <header>
        <div>
          ...
          <h3>{preview.matchedRule?.name || "No allocation rule matched"}</h3>
          <p>
            {preview.matchReason ||
              "This income will remain unallocated until you assign it."}
          </p>
        </div>
        <MoneyAmount value={preview.amountMinor} />
      </header>
      {preview.fixedAllocations.length > 0 && (
      ...
      {preview.percentageAllocations.length > 0 && (
        <div className="allocation-group">
          <p>
            Then divide the remaining{" "}
            <MoneyAmount value={preview.distributableAmountMinor} />
          </p>
      ...
      <footer>
        <div>
          <span>Assigned</span>
          <MoneyAmount value={preview.totalAllocatedMinor} />
        </div>
        <div>
          <span>Unallocated</span>
          <MoneyAmount value={preview.unallocatedAmountMinor} />
        </div>
      </footer>
