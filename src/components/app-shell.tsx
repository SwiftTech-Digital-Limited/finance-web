...
import {
  ArrowLeftRight, BarChart3, ChevronDown, CircleDollarSign, FolderCog,
  Gauge, Landmark, LayoutGrid, ListTree, LogOut, Menu, MoreHorizontal,
  PiggyBank, Plus, ReceiptText, Settings, SlidersHorizontal, Target, X,
} from "lucide-react";
...
const nav = [
  ...
  { label: "Planning", items: [
    { href: "/income-sources", label: "Income sources", icon: CircleDollarSign },
  ...
  return (
    <div className="app-frame">
      <aside className={cn("sidebar", menuOpen && "sidebar-open")}>
        <div className="sidebar-brand"><Link href="/dashboard">{PRODUCT_NAME}<span>.</span></Link><button className="mobile-close" onClick={() => setMenuOpen(false)} aria-label="Close menu"><X /></button></div>
        ...
        <div className="sidebar-footer">
          <Link className={cn("nav-link", pathname.startsWith("/settings") && "active")} href="/settings"><Settings /><span>Settings</span></Link>
          <button className="user-chip" onClick={() => void logout()}><span>{user.name.slice(0, 1).toUpperCase()}</span><span><b>{user.name}</b><small>{user.email}</small></span><LogOut aria-hidden="true" /></button>
        </div>
      </aside>
      ...
      <div className="app-main">
        ...
        <footer className="app-footer"><span>{PRODUCT_NAME} keeps account and bucket ledgers distinct.</span><span>Built with care in Nigeria.</span></footer>
      </div>
      <nav className="mobile-nav" aria-label="Mobile navigation">
        <MobileLink href="/dashboard" label="Home" icon={LayoutGrid} active={pathname.startsWith("/dashboard")} />
        <MobileLink href="/transactions" label="Activity" icon={ReceiptText} active={pathname.startsWith("/transactions")} />
        <Link href="/add" className="mobile-add"><Plus /><span>Add</span></Link>
        <MobileLink href="/rules" label="Plan" icon={SlidersHorizontal} active={pathname.startsWith("/rules") || pathname.startsWith("/monthly-spending")} />
        <MobileLink href="/settings" label="More" icon={MoreHorizontal} active={pathname.startsWith("/settings")} />
      </nav>
    </div>
  );
}
...
function MobileLink({ href, label, icon: Icon, active }: { href: string; label: string; icon: typeof Menu; active: boolean }) {
  return <Link className={active ? "active" : ""} href={href}><Icon /><span>{label}</span></Link>;
}
